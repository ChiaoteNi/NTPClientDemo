...
struct OriginNTPPacket {
    ...
    init(data: Data, destinationTime: TimeInterval) throws {
        ...
        self.rootDelay = OriginNTPPacket.intervalFromNTPFormat(data.getUnsignedInteger(at: 4))
        self.rootDispersion = OriginNTPPacket.intervalFromNTPFormat(data.getUnsignedInteger(at: 8))
        self.clockSource = ClockSource(stratum: self.stratum, sourceID: data.getUnsignedInteger(at: 12))
        self.referenceTime = OriginNTPPacket.dateFromNTPFormat(data.getUnsignedLong(at: 16))
        self.originTime = OriginNTPPacket.dateFromNTPFormat(data.getUnsignedLong(at: 24))
        self.receiveTime = OriginNTPPacket.dateFromNTPFormat(data.getUnsignedLong(at: 32))
        self.transmitTime = OriginNTPPacket.dateFromNTPFormat(data.getUnsignedLong(at: 40))
        self.destinationTime = destinationTime
    }

    /// Convert this OriginNTPPacket to a buffer that can be sent over a socket.
    ///
